(function () {
    ...
    const cssstyle = `
    .sp-aw { text-align: center !important; list-style: none; } @media all and (min-width:940px) { ._CmxRiim30CrxSPL{ width: 965px!important; } } @media all and (min-width:729px) and (max-width:939px){ ._CmxRiim30CrxSPL{ width: 640px !important; } } @media all and (max-width:559px) { ._CmxRiim30CrxSPL{ min-width: 100% ; max-width: 100% ; width: 100% ; } 
    ...
    <div style="padding-left: 5px; font-size: 14px; padding-right: 5px;"></div>
        ...
        <header class="" style="margin-left: 8px;">
            <div class="css-1dbjc4n r-18u37iz" style="cursor:auto;flex-direction: column;margin-left: -8px;">
                ...
                <div style="display: flex;flex-direction: row;">
                    <div style="height: 23px;background-color: #7a7a7a;width: 120px;">
                        <div user-cratel style="display:flex;">
                            <img icon style=" height: 22px; ">
                            <div name style=" padding-left: 6px; "></div>
                        </div>
                    </div>
                    <div style="height: 23px; background-color: red; width: 130px; display: flex; position: relative; text-align: center; border-radius: 6px;"><span style="margin-left: 12px; color: #fff; line-height: 22px; width: 105px;">Follow count and button</span><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 512" style="height: 16px; width: 16px; transform: rotate(45deg); fill: #fff; position: absolute; margin: 0.5px; left: 0px; top: 4px;"><!--! Font Awesome Free 6.2.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free (Icons: CC BY 4.0, Fonts: SIL OFL 1.1, Code: MIT License) Copyright 2022 Fonticons, Inc. --><path d="M54.2 202.9C123.2 136.7 216.8 96 320 96s196.8 40.7 265.8 106.9c12.8 12.2 33 11.8 45.2-.9s11.8-33-.9-45.2C549.7 79.5 440.4 32 320 32S90.3 79.5 9.8 156.7C-2.9 169-3.3 189.2 8.9 202s32.5 13.2 45.2 .9zM320 256c56.8 0 108.6 21.1 148.2 56c13.3 11.7 33.5 10.4 45.2-2.8s10.4-33.5-2.8-45.2C459.8 219.2 393 192 320 192s-139.8 27.2-190.5 72c-13.3 11.7-14.5 31.9-2.8 45.2s31.9 14.5 45.2 2.8c39.5-34.9 91.3-56 148.2-56zm64 160c0-35.3-28.7-64-64-64s-64 28.7-64 64s28.7 64 64 64s64-28.7 64-64z"></path></svg></div>
                    <div style="height: 23px; background-color: #7a7a7a; width: 30px; color: white; text-shadow: 0.07em 0 black, 0 0.07em black, -0.07em 0 black, 0 -0.07em black; padding-left: 4px; padding-right: 4px;" alt=" or NSFW or spoiler">HD</div>
                </div>
            </div>
        </header>
    ...
    <div postviewercontroller="right"></div>
    </div>
    `;
    ...
    document.addEventListener('DOMContentLoaded', async function () {
        ...
        await createscript([APIBASE_JS+'splide.min.js', APIBASE_CSS+'splide.min.css', APIBASE_CSS+'plyr.css', APIBASE_JS+'hls.js', APIBASE_JS+'plyr.js',APIBASE_JS+'marked.min.js','https://cdn.quilljs.com/1.3.6/quill.snow.css','https://cdn.quilljs.com/1.3.6/quill.js']);
    ...
    postjson.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
            ...
            document.querySelector('[user-cratel] [name]').innerHTML = pjson.author.name;
            document.querySelector('[user-cratel] [icon]').src = pjson.author.icon;
            buildmedia(pjson)
            buildmenu(pjson)
            buildcomments(pjson)
            if (typeof sidebar !== 'undefined') {
                ...
                document.querySelector('[postviewercontroller="right"]').style.display = 'none'
            }
    ...
    function buildmedia(data) {
        ...
        if (data.video) {
            setTimeout(() => {
                ...
                const source = 'https://tc-1.nrrinc.net/m3u8/?media=' + data.video.code
                ...
                case 'px1':
                    ...
                    base.querySelector('#community').addEventListener('change', (event) => {
                        if (event.currentTarget.checked) {
                            document.querySelector('._1lDGFVGU9k2mi-4kjU95Rp').innerHTML = 'Next'
                            for (let index = 0; index < radios.length; index++) {
                                const element = radios[index];
                                element.disabled = true;
                            }
                            status.setAttribute('where','next')
                        }else{
                            document.querySelector('._1lDGFVGU9k2mi-4kjU95Rp').innerHTML = 'Submit'
                            for (let index = 0; index < radios.length; index++) {
                                const element = radios[index];
                                element.disabled = false;
                            }
                            status.setAttribute('where','submit')
                        }
                    })
                    document.querySelector('._1lDGFVGU9k2mi-4kjU95Rp').addEventListener('click',function(e) {
                        const base = document.querySelector('[nrrinc-modal]');
                        const radios = base.querySelectorAll('input[type="radio"]')
                        const status = document.querySelector('.cR6KACDFv5FmLxCEFk1_l')
                        if (status.getAttribute('where') == 'next') {
                            document.querySelector('[report="sitewide"]').style.display = 'none'
                            document.querySelector('[report="communitywide"]').style.display = 'block'
                            status.setAttribute('where','submit')
                            document.querySelector('._1lDGFVGU9k2mi-4kjU95Rp').innerHTML = 'Submit'
                        }else{
