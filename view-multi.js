...
function buildvideo(poster,m3u8) {
    ...
    var a = document.createElement('script');
    a.setAttribute('src','https://nrrinc.net/api/v1/cdn/plyr.js');
    a.setAttribute('id',"splidejsid")
    a.setAttribute('integrity','sha384-uax8bmVz/F8mujhaWffZ88h+z0gLEpiA0cwr3YrDKKZ+067Gp0KNRvHbQkjLSUQj')
    a.setAttribute('crossorigin','anonymous')
    document.head.appendChild(a);
    document.head.insertAdjacentHTML('beforeend','<link rel="stylesheet" href="https://nrrinc.net/api/v1/cdn/plyr.css" />');
    var b = document.createElement('script');
    b.setAttribute('src','https://nrrinc.net/api/v1/cdn/hls.js');
    b.setAttribute('id',"splidejsid")
...
function showpost(e,post_id,post_type,json) {
    if (e === '0') {
        showpost = function() {};
        //bricks showpost(). disable for testing
        return;
    }
    ...
    if (window.innerWidth < 559 || urlq[3] == "p") {
      //url example: https://example.com/p/1234-content
        return;
    }
    ...
    if (    document.getElementById('splidecss')) {
        document.getElementById('splidecss').remove()
        document.getElementById("splidejsid").remove()
    }
    ...
    postjson.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
           ...
           window.history.pushState(pjson.title + ' - Dino Portal', pjson.title + ' - Dino Portal', `/p/${pjson.post_id}`);
            ...
            var template = `
            <div dclass="rpBJOHq2PR60pnwJlUyP0" class="ijwijhfiwjifjwijfjiwjfijwsijifjw">
            ...
            <div class="interactive_0" class="interactive_0" style="margin-left: 8px;margin-right: 8px;">
                <div>
                    ...
                    @media all and (min-width:729px) {
                        ...
                        .ijwijhfiwjifjwijfjiwjfijwsijifjw{margin-top: 60px;}
        ...
        `;
        pv.innerHTML = template
        document.getElementById('cvh').innerHTML = comment;
        if (window.localStorage.getItem("betafeatures") === "true") {
            if (!window.innerWidth < 559) {
                document.querySelector('.ijwijhfiwjifjwijfjiwjfijwsijifjw > article').style.cssText  = "width:"+(document.querySelector('._1tvThPWQpORoc2taKebHxs ').offsetWidth + document.querySelector('._1OVBBWLtHoSPfGCRaPzpTf ').offsetWidth) + "px !important;"   
            }
        }
        window.addEventListener("storage", function () {
            if (window.localStorage.getItem("betafeatures") === "true") {
                ...
                document.querySelector('.ijwijhfiwjifjwijfjiwjfijwsijifjw > article').style.cssText  = ""   
            }
        }, false);
        ...
        document.querySelector(`.ijwijhfiwjifjwijfjiwjfijwsijifjw`).style.marginBottom = document.querySelector(`[load-post-id='${pjson.post_id}']`).scrollHeight + 60 + 'px'
        }, 50);
        }}
    postjson.open('GET', 'https://nrrinc.net/api/v1/?k=loadpid&pid=' + post_id + '&pagetype=trending&currenttime=' + Date.now(), true);
    postjson.send();
}
...
function deletecomment(e) {
    e.preventDefault();
    console.log(this.getAttribute("del-id"))
...
function comments(post_id) {
    ...
    ;((document, window) => {
        ...
        function listComment() {
            fetch('https://nrrinc.net/api/v1/?k=postcomments&pid='+post_id)
             .then(response => {
                $list.html("");
            ...
            h.open("GET", 'https://nrrinc.net/api/v1/?k=postcomments&pid='+post_id+"&count", true);
            h.send();
        }
        ...
        const submitHandler = e => {
            ...
            $message.html("");
            fetch("https://nrrinc.net/api/v1/?k=submitcomment&pid="+post_id, {
                method: 'POST',
...
function vmlike() {
    ...
    request.open('POST', 'https://nrrinc.net/api/v1/?k=like&time'+Date.now(), true);
    ...
    request.onload = function () {
        if (this.status >= 200 && this.status < 400) {
            ...
            if (action == "like") {
                ...
                document.querySelector(`[elike-id='${post_id}'] > i > i`).style.color = "unset"
